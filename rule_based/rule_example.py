...
batch = 0
dataparse = lambda dates: pd.datatime.strptime(dates,'%Y-%m-%d %H')
for df in pd.read_csv(open(r"F:\tianchi\fresh_comp_offline\fresh_comp_offline\tianchi_fresh_comp_train_user.csv",'r'),
    ...
    try:
        ...
        df_act_34.to_csv(r"F:\tianchi\fresh_comp_offline\fresh_comp_offline\act_34.csv",
...
'''
data_file = open(r"F:\tianchi\fresh_comp_offline\fresh_comp_offline\act_34.csv",'r')
...
print(df_act_34.columns)
df_time_3 = df_act_34[df_act_34['behavior_type'].isin([3])][['user_id','item_id','time']]           # 3/4 is num,not str
df_time_4 = df_act_34[df_act_34['behavior_type'].isin([4])][['user_id','item_id','time']]
print(df_time_3.shape)
print(df_time_4.shape)
...
#for predict
df_time_3 = df_time[df_time['time4'].isnull()].drop(['time4'],axis = 1)                           #choose the df_time not df_time_34 ,because not dropna
df_time_3 = df_time_3.dropna()
df_time_3.to_csv(r"F:\tianchi\fresh_comp_offline\fresh_comp_offline\time_3.csv",
                 columns = ['user_id','item_id','time3'],
                 index = False)
#save middle data set
df_time_34.to_csv(r"F:\tianchi\fresh_comp_offline\fresh_comp_offline\time_34.csv",
...
data_file = open(r"F:\tianchi\fresh_comp_offline\fresh_comp_offline\time_34.csv",'r')
...
data_file = open(r"F:\tianchi\fresh_comp_offline\fresh_comp_offline\time_3.csv",'r')
try:
...
#generate from P
data_file = open(r"F:\tianchi\fresh_comp_offline\fresh_comp_offline\tianchi_fresh_comp_train_item.csv",'r')
try:
    df_item = pd.read_csv(data_file,
                          index_col=False)
finally:
    data_file.close()

ui_pred_in_P = pd.merge(df_item,ui_pred,on = ['item_id'],how = 'inner')
ui_pred_in_P = ui_pred_in_P.drop_duplicates(['item_id','user_id'])

#user_id - item_id to csv file
ui_pred_in_P.to_csv(r"F:\tianchi\fresh_comp_offline\fresh_comp_offline\tianchi_mobile_recommendation_predict.csv",
